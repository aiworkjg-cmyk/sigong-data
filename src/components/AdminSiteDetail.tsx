        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-5 text-sm">
          ...
          <div className="bg-blue-50/70 p-4 rounded-xl border border-blue-200 sm:col-span-3">
            ...
            <div className="mt-2 text-[11px] text-blue-800 flex flex-wrap items-center justify-between gap-2">
              <span>{site.sharePointDetails?.message || 'SharePoint 전용 폴더에 성공적으로 동기화되었습니다.'}</span>
              {site.sharePointDetails?.syncedAt && (
                <span>동기화 시간: {new Date(site.sharePointDetails.syncedAt).toLocaleTimeString('ko-KR')}</span>
              )}
            </div>
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    className="aspect-video w-full bg-slate-900 relative cursor-pointer overflow-hidden flex items-center justify-center"
                  >
                    {isImg ? (
                      <img
                        src={file.thumbnailUrl || file.storagePath}
                      ...
                      <div className="text-slate-400 text-xs">파일</div>
                    )}
