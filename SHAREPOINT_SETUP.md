...
이 문서는 `.env`의 `SHAREPOINT_*` 값을 채워서 실제 Microsoft SharePoint(Teams 채널의 클라우드 저장소)에
...
(로컬 `data/sharepoint_storage` 폴더에 SharePoint와 동일한 폴더 구조로 저장됨).
...
3. 등록 완료 후 개요(Overview) 화면에서 다음 두 값을 복사해 `.env`에 저장:
...
3. 생성 직후 표시되는 **값(Value)**을 즉시 복사 (페이지를 벗어나면 다시 볼 수 없음) → `.env`의 `SHAREPOINT_CLIENT_SECRET`
...
`.env`의 `SHAREPOINT_SITE_ID`에 그대로 넣습니다. (`SHAREPOINT_DRIVE_ID`는 비워둬도 됩니다 —
...
```
GET http://localhost:3000/api/sharepoint/status
```
...
문제가 있다면 관리자 화면의 "재동기화" 버튼으로 개별 현장 건을 다시 시도할 수 있습니다
(`POST /api/sites/:id/retry-sharepoint`).
