...
| 권한 | 할 수 있는 일 | 어디서 만드나 |
|------|---------------|---------------|
| **마스터** | 전부 + 계정 추가·삭제·권한 변경 | `.env` (또는 App Service 환경변수) |
| **관리자** | 자료·이슈·시공종류 설정 변경 (계정 관리는 불가) | 마스터가 [계정 관리] 화면에서 생성 |
| **일반** | 조회만 (버튼이 아예 표시되지 않고, 서버도 거부) | 마스터가 [계정 관리] 화면에서 생성 |
...
- 관리자 계정은 마스터 1개 + 화면에서 추가 생성하며, 관리자/일반 권한을 지정할 수 있습니다.
